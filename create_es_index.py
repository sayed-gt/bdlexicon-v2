...
def normalize(doc, collection_name, es_index):
    def safe_date(value):
...
def index_to_elasticsearch(args):
    ...
    with client.start_session() as session:
        for col in collections:
            try:
                ...
                for doc in cursor:
                    try:
                        action = normalize(doc, col, ES_INDEX)
                        batch.append(action)
                    except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mongo_uri", type=str, required=True, help="MongoDB URI")
    parser.add_argument("--db_name", type=str, required=True, help="MongoDB database name")
    parser.add_argument("--es_index", type=str, required=True, help="Elasticsearch index name")
    parser.add_argument("--es_uri", type=str, required=True, help="Elasticsearch host URL")
    parser.add_argument("--batch_size", type=int, default=1000, help="Batch size for indexing")
    args = parser.parse_args()
    index_to_elasticsearch(args=args)